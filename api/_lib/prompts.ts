// Shared question/answer mapping and agent prompt builders
// Extracted from api/analyze-brand.ts for reuse across agents

export const QUESTION_MAP: Record<string, Record<string, { question: string; options: Record<string, string> }>> = {
  fmcg: {
...
export const STAGE_QUESTION_IDS: number[][] = [
  [3, 4, 5],
  [9, 10, 11],
  [15, 16, 17],
  [21, 22, 23],
...
// Resolve raw wizard answers into human-readable Q&A text grouped by stage
  ...
  return STAGE_QUESTION_IDS.map((qIds, stageIdx) => {
    ...
    const qaPairs = qIds
      .map((qId) => {
        ...
        const scorePart = score !== undefined ? ` (Skor: ${score})` : '';
