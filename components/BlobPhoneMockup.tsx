...
interface BlobPhoneMockupProps {
  videoSrc: string;
  previewSrc?: string;
  className?: string;
  altText: string;
...
const BlobPhoneMockup: React.FC<BlobPhoneMockupProps> = ({ 
  videoSrc, 
  previewSrc,
  className, 
  altText,
  ...
  useEffect(() => {
    if (!lazy || !containerRef.current || priority) return;
  ...
  // Use preview for initial display, full video for playback
  const currentVideoSrc = previewSrc && !isPlaying ? previewSrc : videoSrc;
          ...
          <div className="absolute inset-[3px] rounded-[2.4rem] overflow-hidden bg-black">
            ...
            {isInView && !videoError && (
              <video
                ref={videoRef}
                key={currentVideoSrc}
                className={`w-full h-full object-contain transition-opacity duration-500 ${videoLoaded && !isLoading ? 'opacity-100' : 'opacity-0'}`}
                src={currentVideoSrc}
                loop
                muted
                playsInline
...
export default BlobPhoneMockup;